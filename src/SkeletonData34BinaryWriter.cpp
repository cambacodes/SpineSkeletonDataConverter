...
int findBinarySkinIndex(const SkeletonData& skeletonData, const std::string& name) {
    // Only a populated default skin occupies an index, and it is always first.
    auto defaultSkin = std::find_if(skeletonData.skins.begin(), skeletonData.skins.end(), [](const Skin& skin) {
        return skin.name == "default";
    });
    bool hasDefault = defaultSkin != skeletonData.skins.end() && !defaultSkin->attachments.empty();
    if (name == "default" && hasDefault) return 0;
    int index = hasDefault ? 1 : 0;
    for (const auto& skin : skeletonData.skins) {
        if (skin.name == "default") continue;
        if (skin.name == name) return index;
        index++;
    }
    throw std::runtime_error("Missing binary skin: " + name);
}
...
void writeVertices(Binary& binary, const std::vector<float>& vertices, bool weighted) {
    if (!weighted) {
        ...
        while (verticesIdx < vertices.size()) {
            int boneCount = (int)vertices.at(verticesIdx++);
            writeVarint(binary, boneCount, true);
            for (int ii = 0; ii < boneCount; ii++) {
                writeVarint(binary, (int)vertices.at(verticesIdx++), true);
                writeFloat(binary, vertices.at(verticesIdx++));
                writeFloat(binary, vertices.at(verticesIdx++));
                writeFloat(binary, vertices.at(verticesIdx++));
            }
        }
...
void writeCurve(Binary& binary, const TimelineFrame& frame) {
    ...
    if (frame.curveType == CurveType::CURVE_BEZIER) {
        writeFloat(binary, frame.curve.at(0));
        writeFloat(binary, frame.curve.at(1));
        writeFloat(binary, frame.curve.at(2));
        writeFloat(binary, frame.curve.at(3));
    }
}
...
void writeAnimation(Binary& binary, const Animation& animation, const SkeletonData& skeletonData) {
        ...
        for (const auto& [timelineName, timeline] : multiTimeline) {
            ...
            switch (timelineType) {
                ...
                case SlotTimelineType::SLOT_RGB:
                    ...
                    break;
                default:
                    throw std::runtime_error("Unsupported slot timeline for Spine 3.3/3.4");
            }
    ...
    for (const auto& [boneName, multiTimeline] : animation.bones) {
        ...
        writeVarint(binary, multiTimeline.size() - multiTimeline.count("inherit"), true);
        for (const auto& [timelineName, timeline] : multiTimeline) {
            ...
            switch (timelineType) {
                ...
                case BONE_SHEARY:
                    ...
                    break;
                default:
                    throw std::runtime_error("Unsupported bone timeline");
            }
            writeVarint(binary, timeline.size(), true);
            switch (timelineType) {
                case BONE_ROTATE: {
    ...
    for (const auto& [skinName, skinMap] : animation.attachments) {
        writeVarint(binary, findBinarySkinIndex(skeletonData, skinName), true);
        ...
        for (const auto& [slotName, slotMap] : skinMap) {
            ...
            writeVarint(binary, slotIndex, true);
            writeVarint(binary, std::count_if(slotMap.begin(), slotMap.end(), [](const auto& attachment) {
                return attachment.second.contains("deform");
            }), true);
...
Binary writeBinaryData(SkeletonData& skeletonData) {
    ...
    if (skeletonData.nonessential) {
        writeString(binary, skeletonData.imagesPath.value_or(""));
    }
    ...
    for (const BoneData& bone : skeletonData.bones) {
        ...
        writeBoolean(binary, bone.inherit != Inherit_OnlyTranslation && bone.inherit != Inherit_NoRotationOrReflection);
        writeBoolean(binary, bone.inherit == Inherit_Normal || bone.inherit == Inherit_NoRotationOrReflection);
        if (skeletonData.nonessential) {
    ...
    if (defaultSkin != skeletonData.skins.end()) writeSkin(binary, *defaultSkin, skeletonData, true);
    else writeVarint(binary, 0, true);
    writeVarint(binary, skeletonData.skins.size() - (defaultSkin != skeletonData.skins.end()), true);
    for (const Skin& skin : skeletonData.skins) {
        if (skin.name != "default") writeSkin(binary, skin, skeletonData, false);
    }
