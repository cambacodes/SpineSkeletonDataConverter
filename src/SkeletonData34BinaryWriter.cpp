...
bool isSupportedAttachment(const Attachment& attachment) {
    return attachment.type >= AttachmentType_Region && attachment.type <= AttachmentType_Path;
}

bool isSupportedSlotTimeline(const std::string& name) {
    const auto it = slotTimelineTypeMap.find(name);
    if (it == slotTimelineTypeMap.end()) return false;
    return it->second == SLOT_ATTACHMENT || it->second == SLOT_RGBA || it->second == SLOT_RGB;
}

bool isSupportedBoneTimeline(const std::string& name) {
    const auto it = boneTimelineTypeMap.find(name);
    ...
    for (const auto& [slotName, slotMap] : skin.attachments) {
        ...
        for (const auto& [attachmentName, attachment] : slotMap) {
            ...
            switch (attachment.type) {
                ...
                default:
                    throw std::runtime_error("Unsupported Spine 3.4 attachment type");
            }
        }
    }
}
...
void writeAnimation(Binary& binary, const Animation& animation, const SkeletonData& skeletonData) {
    ...
    const size_t slotCount = std::count_if(animation.slots.begin(), animation.slots.end(), [](const auto& entry) {
        return std::any_of(entry.second.begin(), entry.second.end(), [](const auto& timeline) {
            return isSupportedSlotTimeline(timeline.first);
        });
    });
    writeVarint(binary, slotCount, true);
    for (const auto& [slotName, multiTimeline] : animation.slots) {
        const size_t timelineCount = std::count_if(multiTimeline.begin(), multiTimeline.end(), [](const auto& entry) {
            return isSupportedSlotTimeline(entry.first);
        });
        ...
        for (const auto& [timelineName, timeline] : multiTimeline) {
            if (!isSupportedSlotTimeline(timelineName)) continue;
            ...
            switch (timelineType) {
                ...
                case SlotTimelineType::SLOT_RGB:
                    ...
                    break;
                default:
                    throw std::runtime_error("Unsupported Spine 3.4 slot timeline type");
            }
    ...
    const size_t boneCount = std::count_if(animation.bones.begin(), animation.bones.end(), [](const auto& entry) {
        return std::any_of(entry.second.begin(), entry.second.end(), [](const auto& timeline) {
            return isSupportedBoneTimeline(timeline.first);
        });
    });
    writeVarint(binary, boneCount, true);
    for (const auto& [boneName, multiTimeline] : animation.bones) {
        const size_t timelineCount = std::count_if(multiTimeline.begin(), multiTimeline.end(), [](const auto& entry) {
            return isSupportedBoneTimeline(entry.first);
        });
        if (timelineCount == 0) continue;
        int boneIndex = 0; 
        ...
        writeVarint(binary, timelineCount, true);
        for (const auto& [timelineName, timeline] : multiTimeline) {
            if (!isSupportedBoneTimeline(timelineName)) continue;
            ...
            switch (timelineType) {
                ...
                case BONE_SHEARY:
                    ...
                    break;
                default:
                    throw std::runtime_error("Unsupported Spine 3.4 bone timeline type");
            }
            writeVarint(binary, timeline.size(), true);
            switch (timelineType) {
                case BONE_ROTATE: {
    ...
    const auto deformCount = [](const auto& map) {
        return std::count_if(map.begin(), map.end(), [](const auto& entry) {
            return entry.second.contains("deform");
        });
    };
    const auto deformSlotCount = [&](const auto& map) {
        return std::count_if(map.begin(), map.end(), [&](const auto& entry) {
            return deformCount(entry.second) > 0;
        });
    };
    const size_t deformSkinCount = std::count_if(animation.attachments.begin(), animation.attachments.end(), [&](const auto& entry) {
        return deformSlotCount(entry.second) > 0;
    });
...
Binary writeBinaryData(SkeletonData& skeletonData) {
    ...
    for (const BoneData& bone : skeletonData.bones) {
        ...
        const bool inheritRotation = bone.inherit == Inherit_Normal || bone.inherit == Inherit_NoScale ||
            bone.inherit == Inherit_NoScaleOrReflection;
        const bool inheritScale = bone.inherit == Inherit_Normal || bone.inherit == Inherit_NoRotationOrReflection;
        writeBoolean(binary, inheritRotation);
        writeBoolean(binary, inheritScale);
        if (skeletonData.nonessential) {
    ...
    /* Skins */
    bool wroteDefaultSkin = false;
    for (const Skin& skin : skeletonData.skins) {
        if (skin.name == "default") {
            writeSkin(binary, skin, skeletonData, true);
            wroteDefaultSkin = true;
            break;
        }
    }
    if (!wroteDefaultSkin) writeVarint(binary, 0, true);
    const size_t nonDefaultSkinCount = std::count_if(skeletonData.skins.begin(), skeletonData.skins.end(), [](const Skin& skin) {
