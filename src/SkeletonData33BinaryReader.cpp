...
SkeletonData readBinaryData(const Binary& binary) {
    ...
    if (skeletonData.nonessential) skeletonData.imagesPath = readString(&input);
    ...
    for (int i = 0; i < numBones; i++) {
        ...
        const bool inheritRotation = readBoolean(&input);
        const bool inheritScale = readBoolean(&input);
        if (inheritRotation && inheritScale) boneData.inherit = Inherit_Normal;
        else if (!inheritRotation && !inheritScale) boneData.inherit = Inherit_OnlyTranslation;
        else if (!inheritRotation) boneData.inherit = Inherit_NoRotationOrReflection;
        else boneData.inherit = Inherit_NoScale;
        if (skeletonData.nonessential) {
            Color color = readColor(&input);
    ...
    if (input.cursor != input.end) {
        throw std::runtime_error("Unexpected trailing data in Spine 3.3 binary");
    }
