...
void writeTimeline(const std::vector<TimelineFrame>& timeline, Json& j, int valueNum, const std::string& key1, const std::string& key2) {
...
Json writeJsonData(const SkeletonData& skeletonData) {
    ...
    for (const auto& bone : skeletonData.bones) {
        ...
        if (bone.inherit == Inherit_OnlyTranslation || bone.inherit == Inherit_NoRotationOrReflection)
            boneJson["inheritRotation"] = false;
        if (bone.inherit == Inherit_OnlyTranslation || bone.inherit == Inherit_NoScale || bone.inherit == Inherit_NoScaleOrReflection)
            boneJson["inheritScale"] = false;
        ...
        if (!animation.bones.empty()) {
            for (const auto& [boneName, boneMap] : animation.bones) {
                ...
                if (boneMap.contains("rotate")) {
                    writeTimeline(boneMap.at("rotate"), boneJson["rotate"], 1, "angle", "");
                }
                if (boneMap.contains("translate")) {
                    writeTimeline(boneMap.at("translate"), boneJson["translate"], 2, "x", "y");
                }
                if (boneMap.contains("scale")) {
                    writeTimeline(boneMap.at("scale"), boneJson["scale"], 2, "x", "y");
                }
                if (boneMap.contains("shear")) {
                    writeTimeline(boneMap.at("shear"), boneJson["shear"], 2, "x", "y");
                }
        ...
        if (!animation.path.empty()) {
            for (const auto& [pathName, pathMap] : animation.path) {
                ...
                if (pathMap.contains("position")) {
                    writeTimeline(pathMap.at("position"), pathJson["position"], 1, "position", "");
                }
                if (pathMap.contains("spacing")) {
                    writeTimeline(pathMap.at("spacing"), pathJson["spacing"], 1, "spacing", "");
                }
